...
void client(void)
{
    ...
    /*
     *
    * 4 is ipV4, then shifted to the left by 4 bits and divided by size of struct ip_header and unsigned long = 5
    *       (64)            (5)
    * 0b 0 1 0 0 0 0 0 | 0b 0 1 0 1 > 0 1 0 0 0 1 0 1 = 69 (0x45)
    * 5 * 4 = 20 (header length)
    *
     * */

    
    /* The total length of header: ip_header + udp_header + length of payload */
    ...
    udp_buf.ip_hdr.iph_ttl = 16; /* Time to live: 16 passes until discarded. 16 - random number */
    udp_buf.ip_hdr.iph_protocol = IPPROTO_UDP; /* Transport protocol */
    udp_buf.ip_hdr.iph_chksum = 0; /* Checksum is calculated automatically */
    udp_buf.ip_hdr.iph_source = inet_addr("127.0.0.1"); /* Source address of client */
    udp_buf.ip_hdr.iph_dest = inet_addr("127.0.0.1"); /* Destination address of the recepient (server) */
    ...
    while (TRUE)
    {
        ...
        {
            printf(" checksum: 0x%x;\n", converted_checksum); /* checksum from the server */
            printf(" source ip address: %s\n", converted_src_ip); /* ip address of the server */
